...
import static drman.support.FilesystemUtils.prepareCandidateBinFolder
import static drman.support.FilesystemUtils.prepareCandidateWithVersionFolder
...
And(~'^I have a local candidate "([^"]*)" version "([^"]*)" at relative path "([^"]*)"$') { String candidate, String version, String relativePath ->
	def fullPath = new File(drmanBaseDir.absolutePath, relativePath)
...
And(~'^I have configured "([^"]*)" to "([^"]*)"$') { String configName, String flag ->
	def configFile = new File("$drmanDir/etc/config")
