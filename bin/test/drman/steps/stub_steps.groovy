...
import static drman.stubs.HookResponses.*
import static drman.stubs.WebServiceStub.*
import static drman.support.FilesystemUtils.readCurrentFromCandidateFolder
import static drman.support.FilesystemUtils.readVersionsCsvFromCandidateFolder
...
And(~'^an available selfupdate$') { ->
	primeEndpointWithString("/selfupdate?beta=false", 'echo "Successfully upgraded DRMAN."')
}
...
And(~/^the appropriate universal hooks are available for "([^"]*)" version "([^"]*)" on "([^"]*)"$/) { String candidate, String version, String platform ->
	String lcPlatform = UnixUtils.asDRmanPlatform(platform).toLowerCase()
