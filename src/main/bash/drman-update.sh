...
function __drm_update() {
	local candidates_uri="${DRMAN_CANDIDATES_API}/candidates/all"
	__drman_echo_debug "Using candidates endpoint: $candidates_uri"

	local fresh_candidates_csv=$(__drman_secure_curl_with_timeouts "$candidates_uri")
	...
	if [[ "$zsh_shell" == 'true' ]]; then
		...
		cached_candidates=( ${(s:,:)DRMAN_CANDIDATES_CSV} )
	else
		OLD_IFS="$IFS"
		IFS=","
		fresh_candidates=(${fresh_candidates_csv})
		cached_candidates=(${DRMAN_CANDIDATES_CSV})
		IFS="$OLD_IFS"
	fi

	__drman_echo_debug "Local candidates: $DRMAN_CANDIDATES_CSV"
	__drman_echo_debug "Fetched candidates: $fresh_candidates_csv"

	if [[ -n "$fresh_candidates_csv" && -z "$detect_html" ]]; then
		# legacy bash workaround
		if [[ "$bash_shell" == 'true' && "$BASH_VERSINFO" -lt 4 ]]; then
			__drman_legacy_bash_message
			echo "$fresh_candidates_csv" >| "$DRMAN_CANDIDATES_CACHE"
			return 0
		fi

		__drman_echo_debug "Fresh and cached candidate lengths: ${#fresh_candidates_csv} ${#DRMAN_CANDIDATES_CSV}"
